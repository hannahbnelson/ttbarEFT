...
#### TW WC POINTS ####
OG_pt = {"ctGIm": -0.4, "ctGRe":-0.4, "cHQ3":1.0, "ctWRe":-1.0, "cleQt3Re":8.0,
        "cleQt1Re":15.0, "cQl3":10.0, "cbWRe":-5.0, "cHtbRe":5.0}

SMtw_pt = {"ctGIm": 0.0, "ctGRe":0.0, "cHQ3":0.0, "ctWRe":0.0, "cleQt3Re":0.0,
...
class AnalysisProcessor(processor.ProcessorABC):
    def __init__(self, samples, wc_names_lst=[], hist_lst = None, dtype=np.float32, do_errors=False):
        ...
        self._histo_dict = {
            ...
            "weights_stpt"      : Hist(hist.axis.Regular(bins=20, start=-10, stop=10, name="weights_stpt", label='event weights at the starting point'), storage="weight"),
            "weights_stpt_log"  : Hist(hist.axis.Regular(bins=70, start=-6, stop=2, name="weights_stpt_log", label='log(event weights at the starting point)'), storage="weight"),
            "weights_pt1_log"  : Hist(hist.axis.Regular(bins=70, start=-6, stop=2, name="weights_pt1_log", label='log(event weights at pt1)'), storage="weight"),
            "weights_pt3_log"  : Hist(hist.axis.Regular(bins=70, start=-6, stop=2, name="weights_pt3_log", label='log(event weights at pt3)'), storage="weight"),
            "weights_pt5_log"  : Hist(hist.axis.Regular(bins=70, start=-6, stop=2, name="weights_pt5_log", label='log(event weights at pt5)'), storage="weight"),
        }
    ...
    def process(self, events):
        ...
        wc_lst_stpt = order_wc_values(self._wc_names_lst, st_pt)
        event_weights_stpt = calc_event_weights(eft_coeffs, wc_lst_stpt)
        ...
        variables_to_fill = {
            ...
            "weights_stpt"  : event_weights_stpt,
            "weights_stpt_log": np.log10(event_weights_stpt),
