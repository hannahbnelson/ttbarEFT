...
class AnalysisProcessor(processor.ProcessorABC):
    def __init__(self, samples, wc_names_lst=[], hist_lst = None, dtype=np.float32, do_errors=False):
        ...
        self._histo_dict = {
            ...
            "top_eta"      : HistEFT("Events", wc_names_lst, hist.Cat("sample", "sample"), hist.Bin("top_eta", "$\eta$(leading top)", 30, -3, 3)),
            "top_phi"      : HistEFT("Events", wc_names_lst, hist.Cat("sample", "sample"), hist.Bin("top_phi", "$\phi$(leading top)", 30, -np.pi, np.pi)),
            "njets_0p"     : HistEFT("Events", wc_names_lst, hist.Cat("sample", "sample"), hist.Bin("njets_0p", "njets with 0 extra partons in ME", 10, 0, 10))
        }
    ...
    def process(self, events):
        ...
        variables_to_fill = {
            "tops_pt"       : tops_pt,
            "l0pt"          : ak.flatten(l0pt),
            "dr_leps"       : dr_cut,
            "avg_top_pt"    : avg_top_pt,
            "njets"         : njets,
            "nleps"         : nleps,
            "mtt"           : mtt,
            "ht"            : ht,
